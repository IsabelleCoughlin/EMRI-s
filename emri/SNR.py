def snr2_increment(a, e, dt, N_harm_sum):
    SNR2 = 0.0
    for n in range(1, N_harm_sum+1):
        f_n  = n * f_orb(a)            # Hz
        h_nv = h_n(n, e, a)            # strain
...
def snr_total_1yr_vs_time(a_arr, e_arr, t_arr, N_harm_sum):
    ...
    for i in range(T):
        ...
        e  = e_arr[i]
        dt = dt_arr[i]
        SNR2_total[i] = snr2_increment(a, e, dt, N_harm_sum)
